...
#include "ui_recorditeminquiry.h"
...
RecordItemInquiry::RecordItemInquiry(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::RecordItemInquiry)
{
    ui->setupUi(this);
    this->setFixedSize(900,600);

    //4.设置窗口标题
    this->setWindowTitle("电子病历查询系统");

    connect(ui->exitPushbutton,&QPushButton::clicked,this,&RecordItemInquiry::on_pushButton_clicked);
    ...
    connect(ui->inquirepushbutton, &QPushButton::clicked,[=]() {
        ...
        {
            RecordItemPatient *p = new RecordItemPatient;
            p->show();
            this->hide();
            connect(p, &RecordItemPatient::rpClosed, this, &QWidget::show);
        });
    });
...
void RecordItemInquiry::paintEvent(QPaintEvent*)
{
    QPainter painter1;
    painter1.begin(this);
    QPixmap pixmap1;
...
RecordItemInquiry::~RecordItemInquiry()
{
    delete ui;
}


void RecordItemInquiry::on_pushButton_clicked()
{
    emit preToHome();
}
